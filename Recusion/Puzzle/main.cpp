...
void main()
{
	...
	SendMessage(hConsole, CONSOLE_FULLSCREEN, 0, 0);*/
	...
	do
	{
		...
		do
		{
			...
			default: cout << "Нет такого уровня\a" << endl;
			}
		} while ((complexity<'1' || complexity>'3') && complexity != escape);*/
		InitField(field, n/*, 3, 3, 0, mix_iterations*/);
...
void ASCII()
{
	for (int i = 0; i < 256; i++)
	{
		...
		//if (i % 16 == 0)cout << endl;		cout << (char)i << " ";
	}
}
...
void Check(int** field, const int n, int x, int y, int number_of_moves)
{
	...
	//if (field[0][0] != 1)game_over = false;
	int value = 1;
	for (int i = 0; i < n; i++)
	{
